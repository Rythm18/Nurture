...
if (isset($_POST['uname']) && isset($_POST['password'])
	...
	if (empty($uname)) {
		header("Location: index.php?error=E-Mailis required&$user_data");
	    exit();
	}else if(empty($pass)){
        ...
        header("Location: index.phpp?error=Name is required&$user_data");
	    exit();
	}
		...
		if (mysqli_num_rows($result) > 0) {
			header("Location:index.php?error=The Email Is Already Registerd Please Login&$user_data");
	        exit();
		}else {
